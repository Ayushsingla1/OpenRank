...
impl XBetContract {
    ...
    pub fn initialize(env: Env, owner: Address, usd_contract: Address) {
        ...
        env.storage().instance().set(&DataKey::UsdContract, &usd_contract);
    }
    ...
    pub fn stake_amount(
        ...
        let buy_yes      = bet == 1;
        let price        = Self::calculate_delta_cost(game.yes_shares, game.no_shares, buy_yes, shares);
        let token_amount = price as i128;
        ...
        let usd_contract: Address = env
            .storage()
            .instance()
            .get(&DataKey::UsdContract)
            .unwrap();
        ...
        let mut user_bet = env
            ...
            .persistent()
            .get(&bet_key)
            .unwrap_or(Bet { yes_shares: 0, no_shares: 0 });
    ...
    /// Announce the result of a game (owner only).
    ...
    /// Redeem winnings for a finished game.
